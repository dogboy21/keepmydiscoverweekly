        ...
        for (let i = 0; i < playlists.length; i++) {
            (function(index) {
                ...
                checkPlaylist(playlist.token, playlist.id, playlist.checksum, (err, created) => {
                    if (err) {
                        console.log('Failed to check playlist state during householding task', err);
...
app.get('/', utils.simpleTemplateRenderer('index'));
    ...
    spotify.me(accessToken, (err, response) => {
       ...
       spotify.playlists(accessToken, (err, response) => {
           ...
           for (let i = 0; i < playlists.length; i++) {
               ...
               playlistItems.push({uri: playlist.external_urls.spotify, name: playlist.name, id: playlist.id});
           }

           return res.render('config', {avatar: avatar, username: response.display_name || response.id, playlists: playlistItems, playlistsFound: playlistItems.length > 0});
       });
    });
});
    ...
    if (req.query.error === 'access_denied') {
        ...
        spotify.getToken(code, (err, response) => {
            ...
            db.setToken(response.access_token, response.refresh_token, response.expires_in, (err) => {
                ...
                res.cookie('token', response.access_token, {signed: true, maxAge: 60 * 60000, httpOnly: true, secure: config.secureCookies});
